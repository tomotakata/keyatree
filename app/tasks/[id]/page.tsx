...
function ToBadges({ toIds, toNames }: { toIds: string[]; toNames: string[] }) {
  if (toIds[0] === "all") {
    return <span className="inline-flex items-center gap-1 text-xs font-bold bg-teal-50 text-teal-700 border border-teal-200 px-2 py-0.5 rounded-full">全員</span>;
  }
  return (
...
function MessageBubble({
  msg, allMessages, taskId, onReaction, onReply,
}: {
  msg: TaskMessage;
    ...
    <div className={`group flex gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div className={`flex flex-col gap-0.5 max-w-xs lg:max-w-md ${isMe ? "items-end" : "items-start"}`}>
        ...
        <div className={`flex flex-wrap items-center gap-1.5 ${isMe ? "justify-end" : "justify-start"}`}>
          <span className="text-xs font-bold text-gray-600">{msg.subject || "（件名なし）"}</span>
...
function ComposePanel({
  ...
  useEffect(() => {
    if (replyTo) {
      ...
      if (senderId !== CURRENT_USER.id && senderId !== "system") {
        setSelectedIds([senderId]);
      }
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            {showMemberList && (
              <div className="mt-1 border border-gray-200 rounded-xl bg-white shadow-md overflow-hidden">
                {/* 全員 */}
                <button
                  onClick={() => { handleAll(); setShowMemberList(false); }}
  ...
  const handleInvite = (emp: typeof MOCK_EMPLOYEES[number]) => {
    ...
    if (updated) {
      ...
      const withMsg = addMessage(task.id, {
        ...
        subject: "メンバー追加", toIds: ["all"], toNames: ["全員"],
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
              {task.messages.length === 0 && (
                <div className="text-center py-12 text-gray-400">
                  ...
                  <p className="text-xs mt-1">件名と宛先を選んで最初のメッセージを送りましょう</p>
                </div>
              )}
            ...
            {[
              ...
              <button key={action.label}
                onClick={() => {
                  ...
                  const updated = addMessage(task.id, {
                    senderId: CURRENT_USER.id, senderName: CURRENT_USER.name,
                    subject: action.subject, toIds: ["all"], toNames: ["全員"],
                    text: action.text,
                  });
